...
예약 버튼을 누를 필요가 없다. **2026-09-08부로 구글 블로그(화 10:00)와 페이스북·
인스타그램·유튜브쇼츠(목 20:00)가 서로 다른 시각에 발행되도록 2단계 배치로 나뉘었다**
— `scripts/publish/main.py`가 `--phase blog`/`--phase social` 인자로 어느 배치인지
구분한다(§3-1 참고).
...
2. **화요일 10:00 KST**, 예약 발행 워크플로가 `--phase blog`로 실행돼 `queue/` 아래
   ...
   블로그 URL을 참조하므로 항상 먼저 끝나야 한다). 이 시점엔 아직 4채널이 다 끝난
   게 아니므로 큐 폴더는 삭제되지 않는다.
3. **목요일 20:00 KST**, 같은 워크플로가 이번엔 `--phase social`로 실행돼 나머지
   3채널(페이스북·인스타그램·유튜브 쇼츠)에 동시에 게시한다.
4. **4채널 모두 게시에 성공하면**, 그 시점 워크플로가 `queue/<번호>_<주제요약>/`
   폴더를 삭제하는 커밋을 자동으로 만들어 푸시한다 — 깃허브에는 다시 남지 않고,
   로컬 원본만 계속 보관된다.
5. 하나라도 게시에 실패하면 그 채널만 재시도 대상으로 표시하고, **큐 폴더는 삭제하지
   않는다**(성공한 채널까지 기록해두고, 실패한 채널만 다음 실행에서 재시도 — 중복
   게시 방지를 위해 채널별 성공 여부를 큐 폴더 안에 작은 상태 파일로 같이 커밋해둔다).
6. 반자동 채널(네이버 블로그·유튜브 커뮤니티·네이버클립·당근·카카오톡채널·
...
- `scripts/publish/main.py` — 오케스트레이터. `queue/` 폴더 하나를 찾아 6단계
  ...
  인자로 이번 실행이 블로그 단계까지만인지, 소셜 5채널까지인지, 한 번에 전부인지
  고른다(§3-1).
- `scripts/publish/{blogger,facebook,instagram,youtube}.py` — 채널별 API 호출.
- `.github/workflows/publish.yml` — cron 2개(화요일 01:00 UTC=10:00 KST →
  `--phase blog`, 목요일 11:00 UTC=20:00 KST → `--phase social`) +
  `workflow_dispatch`로 phase를 골라 수동 테스트 가능. 실행 후 `_status.json`
  변경이나 큐 폴더 삭제를 항상 커밋·푸시한다(중간에 실패해도 `if: always()`로
  상태는 저장됨).
...
### 3-1. 2단계 배치(`--phase`) 동작 방식 (2026-09-08 신설)

`main.py`가 매번 6채널을 전부 발행하던 것을, 화요일/목요일 두 번의 워크플로 실행에
걸쳐 나눠 처리하도록 바꿨다 — 배경은 §1의 "발행 요일을 정한 근거"(`mainmanager-plan.md`
§1-1)와 동일: 블로그 원문을 먼저 내보내고 이틀 뒤 저녁에 카드뉴스·숏츠로 재환기한다.

- `--phase blog`(화요일): 구글 블로그 단계만 실행하고 끝난다. `_status.json`에
  `blogger`만 기록되므로 `all_steps_done()`이 아직 `False`다 — 큐 폴더는 삭제되지
  않고 다음 배치를 기다린다.
- `--phase social`(목요일): 블로그가 이미 끝나 있어야 한다(`_status.json`에서
  `blogger.url`을 읽어 캡션의 `{{BLOG_URL}}`을 채운다) — 안 끝나 있으면 에러를 내고
  멈춘다(잘못된 순서로 실행됐다는 뜻이므로 조용히 넘어가지 않는다). 나머지 5채널을
  스레드풀로 동시에 발행하고, 전부 끝나면 큐 폴더를 삭제한다.
- `--phase all`(기본값): 예전처럼 한 번에 전부 — 수동 재시도나 dry-run 점검용으로
  남겨뒀다.
- 두 워크플로 실행은 `_status.json`이라는 같은 상태 파일을 통해 이어진다 — 화요일
  실행이 커밋해둔 `_status.json`을 목요일 실행이 그대로 읽어 이어서 처리하는 구조라,
  재시도·부분 실패 로직(§2의 4~5번)이 그대로 두 배치에도 적용된다.

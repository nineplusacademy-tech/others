...
### 2-1. 예약이 지연되거나 실패했을 때 (2026-09-22 신설, 2026-09-23 원인 확정)

**GitHub Actions의 `schedule` cron은 예약 시각을 보장하지 않는다.** 2026-09-22엔
"매시 정각을 피하면 줄어든다"는 GitHub 공식 권고대로 분(minute)을 옮겨봤지만
효과가 없었다 — 9/23에도 :06 예약이 4시간52분 지연됐다. 2주치 실행 기록을 전부
대조한 결과, **예약 시각과 무관하게 거의 매번 4.5~5.3시간(드물게 13~14시간)
지연되는 패턴**을 확인했다 — GitHub Free 플랜의 예약 작업이 전역 "낮은 우선순위
큐"에서 처리되기 때문으로 보이며, 동일 증상을 보고한 사례도 있다
([GitHub 커뮤니티 토론 #201738](https://github.com/orgs/community/discussions/201738):
"cron 분을 바꿔도 지연이 그대로다 — 분과 무관하게 계정 전체가 낮은 우선순위 큐
단위로 일괄 처리되는 것으로 보인다").

**2026-09-23 대응(우회 전략, 실험적)**: 지연폭이 대체로 일정하다는 점을 거꾸로
이용해, `publish.yml`·`publish-edu.yml`·`publish-thread.yml`·`publish-edu-thread.yml`·
`insights.yml`의 예약 시각을 **실제로 노출되길 원하는 시각보다 5시간 앞당겨**
잡아뒀다(예: 화요일 10:00 KST에 뜨길 원하면 월요일 20:00 UTC(=화요일 05:00 KST)에
예약 — 지연을 거치면 원래 원하던 오전 시간대 근처에 실행되길 기대). 각 워크플로
파일 상단 주석에 계산 근거가 있다. **100% 정확히 맞는다는 보장은 아니다** —
지연폭 자체가 흔들리고 드물게 훨씬 더 밀린 전례(13~14시간)도 있어서, 몇 주
지켜보고 필요하면 다시 조정한다. 근본적으로 지연 자체를 없애려면 외부 무료
cron 서비스(예: cron-job.org)가 정확한 시각에 GitHub REST API의
`workflow_dispatch`를 직접 호출하는 방식이 필요하다 — 이건 예약 대기열 자체를
거치지 않아 지연이 없지만, 사용자의 GitHub 개인 액세스 토큰을 그 외부 서비스에
직접 등록해야 하는 단계가 있어 사람이 직접 설정해야 한다.

늦게라도 그날 안에 발행이 안 됐다면(Actions 탭에서 확인) 대응은 둘로 갈린다 —
**절대 섞으면 안 된다**:

- **안전 — 새로 실행**: Actions 탭 → 해당 워크플로 → **Run workflow** 버튼으로 새로
  트리거한다(`workflow_dispatch`). 이건 항상 최신 코드·최신 `_status.json`을 다시
  읽고 시작하므로, 이미 끝난 단계는 건너뛰고 안 끝난 단계만 재시도한다(§2의 6번,
  `main.py`/`main_edu.py`의 `step_done()` 가드) — 여러 번 눌러도 중복 게시되지 않는다.
- **위험 — 과거 실행의 "Re-run failed jobs"**: GitHub Actions의 이 버튼은 그 **실행
  당시 커밋 상태**를 그대로 다시 체크아웃해서 돌린다 — 그 사이 `_status.json`이
  갱신됐어도 반영되지 않는다. 이미 채널에 실제로 게시된(예: Blogger `insert`처럼
  "글은 이미 라이브인데 부가 단계만 실패"한 경우) 실행을 이 버튼으로 재실행하면
  **똑같은 글이 한 번 더 올라간다**(9/22 실제로 이 상태였다가 수동 개입으로 막음).
  실패한 과거 실행은 원인 파악용으로만 열어보고, 재시도는 항상 위 "새로 실행"으로
  한다.

**Blogger 검색설명 보정 강화(2026-09-24)**: 게시 직후 라벨·검색설명 반영 여부를 확인해
빠진 것을 보정하는 시도를 예전 "대기 없이 patch 2회"에서 "2·5·10·20초 간격 대기 후 patch 2회 →
posts.update(PUT) 2회"로 늘렸다(그래도 안 되면 경고만 남기고 성공 처리). 가짜 서비스로
4가지 경우(즉시 반영/patch로 해결/update로 해결/끝내 실패)를 시험해 동작을 확인했지만,
실제 Blogger에서 update 방식이 통하는지는 다음 블로그 발행 때 확인해야 한다.

발행 함수 자체도 이 원칙을 따른다 — 실제로 콘텐츠가 라이브(공개)된 뒤에는 예외를
던지지 않는다(부가 정보가 덜 반영돼도 경고만 남기고 성공으로 처리, `blogger.py`
참고) — 그래야 "게시는 됐는데 실패로 기록됨" 상태 자체가 생기지 않는다.

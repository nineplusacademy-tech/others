...
### 2-1. 예약이 지연되거나 실패했을 때 (2026-09-22 신설)

**GitHub Actions의 `schedule` cron은 예약 시각을 보장하지 않는다** — 매시 정각처럼
전 세계 워크플로가 몰리는 시각에는 몇 시간까지 밀릴 수 있다(2026-09-22 9주차 실사고:
화요일 10:00 KST 예약이던 블로그·교육뉴스 배치가 실제로는 14:30~15:00경에야 실행됨).
`publish.yml`·`publish-edu.yml`·`publish-thread.yml`·`publish-edu-thread.yml`의 cron
분(minute)을 전부 정각(`:00`)에서 몇 분씩 어긋나게 옮겨(GitHub 공식 권고) 빈도는
줄였지만, 완전히 없앨 수는 없다.

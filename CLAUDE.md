...
시작 시 mainmanager가 가장 먼저 호출하며, 데이터랩툴즈 MCP(datalab.tools 크롬
확장)로 실제 방문·유입·체류·재방문 통계와 블로그·플레이스 검색순위(SERP)를
...
- 산출물(마크다운/밴드용/구글블로그 HTML 마스터본 3종 + 채널별 배포본 4종 + 업로드
  체크리스트 1종 + 삽화 1장, 총 9개 파일)은 **이 저장소가 아니라 로컬

"""스레드(메인 블로그) 화요일 10:15 자동 게시 오케스트레이터.

메인 블로그 파이프라인(queue/)과 같은 폴더를 보되, main.py(블로그·카드뉴스·
릴스/쇼츠 3단계)와는 별도로 화요일 10:15 KST(2026-09-13부로 12:30→10:15 —
모든 콘텐츠 업로드를 오전 10시대로 통일, 블로그 URL 기록 직후로 당길 수 있는
최소 버퍼만 남김)에 독립 실행된다(`docs/mainmanager-plan.md` §1-1). 카드뉴스
캐러셀 첨부 없이 텍스트(+ 네이버 블로그 링크)만 자동 게시한다 — 이미지를
첨부하기로 한 주는 당분간 사람이 그대로 수동 게시한다(`content-playbook.md` §9).

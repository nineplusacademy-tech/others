...
Blogger만 먼저 끝내고(다른 채널 캡션이 블로그 URL을 참조하므로), 나머지
5개 채널(FB 게시글·IG 캐러셀·FB 릴스·IG 릴스·YT 쇼츠)은 서로 의존하지
않으므로 스레드풀로 동시에 실행한다 — 특히 인스타 릴스는 처리 완료까지
최대 5분 폴링이라, 순차 실행이면 그 대기 시간이 전체 소요 시간에 그대로
더해진다. 동시 실행하면 전체 소요 시간이 "5개 합산"이 아니라 "5개 중
가장 오래 걸리는 것" 기준으로 줄어든다.
...
def run(folder: Path) -> None:
    ...
    tasks: dict[str, tuple] = {
        "facebook_post": (_facebook_post, "id"),
